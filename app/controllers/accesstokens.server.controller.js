...
var probando = require('../../config/accesstokens.js');

/**
 * Función para probar los scripts internos desde el postman
 */

exports.generaPrueba = function(req, res) {
  // Para obtener el access_token de un usuario
  // probando.obtienePagATByUsrAT('facespa', '254867754693028', '543c0b067a8acbc318999d92', '10153624325708993', 'CAAJyyo8ttgcBAMtAEvBAlBIvCvXK2EUwO0ZAUEbHgDtfULaZAdJpDqrbf2rWZCMaZBvJez1TZC4y6ShBRXAcnzYvTMYyCqMlbLTpfruhpIalNSEP6Vk9bx6nI45b0gvEjWdVNtndt2ZAZA9y3nVmbCeW63Qub104LlsTgEhcltitZCda7DgaZBPDtZCtYEceIse9ZAoPFL9u3KpTwZDZD', 'no', function(respuesta){

  // Para obtener un access_token de una página con base en los usuarios del crm que la administran
  // probando.obtienePagATs('compuycomal', '1759568900936759', 'no', function(respuesta){
  
  // Comprueba si el access_token que tenemos en la base de datos para ese usuario es válido o no para usarlo
  // probando.compruebaUnUsrAT('Ady', 'no', function(respuesta){

  // Para obtener un access_token de usuario de uno de los usuarios de una cuenta del crm
  // probando.obtieneUsrsAT('compuycomal', 'no', function(respuesta){

  // Para obtener un access_token de usuario de uno de los usuarios del crm
  probando.obtieneUsrsATAmongAllUsrs('yes', function(respuesta){

  // Para obtener el access_token de la app
  // probando.obtieneAppAT('no', function(respuesta) {
    res.jsonp(respuesta);
  });
};
 ...
 */
exports.obtieneAppAText = function(req, res) {
  /**
  ...
  function requestAT (callback) {
    ...
    solicitud.on('error', function(erra){
      var objeto = {};
      objeto.error = erra;
      console.log('accesstokens.server.controller/obtieneAppAText/requestAT - error en el request: '+erra);
  ...
  function getAppAT (callback) {
	...
	if (items.length < 1) {
	  ...
	  requestAT(function(at) {
	      ...
	      console.log('accesstokens.server.controller/obtieneAppAText/getAppAT - access_token error en request o venía vacío: '+at);
	      return callback('error');
	    }
	    else {
	      var objeto = {};
	      objeto = { at: 'access_token='+at.access_token, ts: new Date() };
	      classdb.inserta('verifica_at', objeto, 'accesstokens.server.controller/obtieneAppAText/getAppAT', function(respuesta){
	  ...
	  if (tsm1 < ahora) {
	    ...
	    requestAT(function(at) {
	      if (at.error || typeof at === 'undefined' || typeof at.access_token === 'undefined') {
		console.log('accesstokens.server.controller/obtieneAppAText/getAppAT - access_token error en request o venía vacío');
		return callback('error');
	      }
	      else {
		...
		var objeto = {};
		objeto = { at: 'access_token='+at.access_token, ts: new Date() };
		classdb.actualiza('verifica_at', criterio_actualizacion, objeto, 'accesstokens.server.controller/obtieneAppAText/getAppAT', function(respuestau) {
		  return callback('access_token='+at.access_token);
		});
	      }
	    });
...
/**
 * Función para obtener el access token de un usuario al azar (de entre todos los usuarios) de una cuenta específica
 */
exports.obtieneUsrsAText = function(req, res) {
  function obtieneUsuariosDeCuenta(lacta, callback) {
    var userscritere = {
      $and: 
      [
	{'cuenta.marca' : lacta},
  ...
  var cuenta, prefix;
  if ('cuenta' in req.query) {
    cuenta = req.query.cuenta;
    if ('prefix' in req.query) prefix = req.query.prefix;
    obtieneUsuariosDeCuenta(cuenta, function(usuariosdecuenta) {
      if (usuariosdecuenta === 'error') {
        res.jsonp(usuariosdecuenta);
      }
      else {
        getCtaATs(cuenta, usuariosdecuenta, function(tokensdecta){
          if (tokensdecta.length < 1) {
            ...
            if (prefix === 'no') {
              res.jsonp(el_at_elegido);
            }
            else {
              res.jsonp('access_token='+el_at_elegido);
            }
          }
        });
      }
  ...
  else if('cuenta' in req.body) {
    cuenta = req.body.cuenta;
    if ('prefix' in req.body) prefix = req.body.prefix;
    obtieneUsuariosDeCuenta(cuenta, function(usuariosdecuenta) {
      if (usuariosdecuenta === 'error') {
        res.jsonp(usuariosdecuenta);
      }
    ...
    console.log('no recibimos parámetros');
    res.jsonp('error')
  }
};
 ...
 */
exports.compruebaUnUsrAText = function(req, res) {
  function requestGraph(path, callback) {
    if (typeof path !== 'undefined' || path !== null || path !== '') {
      var opciones = globales.options_graph;
      opciones.method = 'GET';
        ...
        revisaUsrATEnMongo(username, eluser[0].additionalProvidersData.facebook.accessToken, function(validat){
          if (validat === 'error') {
            ...
            if (prefix === 'no') {
              ...
              res.jsonp('access_token='+validat);
            }
          }
        });
      }
 ...
 */
exports.obtienePagATsext = function(req, res) {
  /**
  ...
  var cuenta, page_id, prefix;
  if ('cuenta' in req.query && 'page_id' in req.query) {
    cuenta = req.query.cuenta;
	...
	res.jsonp(dieusern);
        ...
        getCtaATs(cuenta, page_id, dieusern, function(tdc){
          if (tdc.length > 0) {
            ...
            if (prefix === 'no') {
              res.jsonp(el_at_elegido.access_token);
            }
            else {
              res.jsonp('access_token='+el_at_elegido.access_token);
            }
          }
          else {
            res.jsonp('no había access_tokens válidos');
          }
        });
      }
    });
  }
  else if ('cuenta' in req.body && 'page_id' in req.body) {
    cuenta = req.body.cuenta;
 ...
 */
exports.obtienePagATByUsrAText = function(req, res) {
  /**
  ...
  var cuenta, page_id, user_aid, fb_usid, us_token, prefix;
  if ('cuenta' in req.query && 'page_id' in req.query && 'user_aid' in req.query && 'fb_usid' in req.query && 'us_token' in req.query) {
    cuenta = req.query.cuenta; page_id = req.query.page_id; user_aid = req.query.user_aid;
    fb_usid = req.query.fb_usid; us_token = req.query.us_token; prefix = req.query.prefix;
    ...
    getUserPageAT(cuenta, page_id, id_de_usuario, fb_usid, us_token, function(pagtoken){
      if (pagtoken === 'error') {
        ...
        if (prefix === 'no') {
          ...
          res.jsonp('access_token='+pagtoken)
        }
      }
    });
  }
  else if ('cuenta' in req.body && 'page_id' in req.body && 'user_aid' in req.body && 'fb_usid' in req.body && 'us_token' in req.body) {
    cuenta = req.body.cuenta; page_id = req.body.page_id; user_aid = req.body.user_aid;
    fb_usid = req.body.fb_usid; us_token = req.body.us_token; prefix = req.body.prefix;
    var id_de_usuario = new ObjectID(user_aid);
    getUserPageAT(cuenta, page_id, id_de_usuario, fb_usid, us_token, function(pagtoken){
      if (pagtoken === 'error') {
        res.jsonp(pagtoken);
