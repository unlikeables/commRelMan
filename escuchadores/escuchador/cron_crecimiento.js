...
function guardaLCSs(suma, fans, callback) {
    ...
    classdb.existecount(collectio, criterius, 'cron_crecimiento/guardaLCSs', function(existe){
	if (existe === 'error') {
	    return callback(existe);
	}
	else if (existe === 'existe') {
	    classdb.buscarToArray(collectio, criterius, {fecha: -1}, 'cron_crecimiento/guardaLCSs', function(elemento){
		if (elemento === 'error') {
		   return callback(elemento);
		}
		else {
		    classdb.actualiza(collectio, criterius, elset, 'cron_crecimiento/guardaLCSs', function(actualizad) {
	    ...
	    classdb.inserta(collectio, objeto, 'cron_crecimiento/guardaLCSs', function(insercion){
...
function guardaRcFcs(suma, followers, tweets, favs, callback) {
    ...
    classdb.existecount(collectio, criterius, 'cron_crecimiento/guardaRcFcs', function(existe){
	if (existe === 'error') {
	    return callback(existe);
	}
	else if (existe === 'existe') {
	    classdb.buscarToArray(collectio, criterius, {fecha: -1}, 'cron_crecimiento/guardaRcFcs', function(elemento){
		if (elemento === 'error') {
		   return callback(elemento);
		}
		else {
...
function obtenLCS(cuenta, fans, callback) {
    ...
    classdb.buscarToArrayFields(cuenta+'_fb_publicaciones', {}, fields, {}, 'cron_crecimiento/obtenLCS', function(items){
...
function obtenRcFcs(cuenta, followers, tweets, favs, callback) {
    ...
    classdb.buscarToArrayFields(cuenta+'_propio', {}, fields, {}, 'cron_crecimiento/obtenRcFc', function(items) {
...
function iteraCuentas(cuentas, index, callback) {
	...
	if (typeof cuentas[index].datosPage !== 'undefined') {
	    ...
	    classdb.buscarToArray(cuentas[index].nombreSistema+'_fans_ahora', {}, {}, 'cron_crecimiento/iteraCuentas', function(items) {
		var fans = 0;
		...
		obtenLCS(cuentas[index].nombreSistema, fans, function(yup){
			...
			classdb.buscarToArray(cuentas[index].nombreSistema+'_twitter_data_ahora', {}, {}, 'cron_crecimiento/iteraCuentas', function(td){
...
function obtieneCrecimientos() {
    ...
    classdb.buscarToArrayFields('accounts', criterio, fields, desort, 'cron_crecimiento/obtieneCrecimientos', function(items){
