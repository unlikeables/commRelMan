...
function revisalast(solicitudes, callback) {
    ...
    classdb.buscarToArray('peticiones_automaticas', {}, {created : -1}, 'accounts/getVariousContents/revisaLast', function(items){
	if (items === 'error') {
	    callback(items);
	}
	else {
		...
		classdb.inserta('peticiones_automaticas', solicitud, 'accounts/getVariousContents/revisaLast', function(insercion){
		    if (insercion === 'error'){
			console.log('accounts/getVariousContents/revisaLast - error al insertar peticion automática');
			callback(solicitud);
		    }
		    else {
			callback(solicitud);
		...
		if (tsm2m < ahora) {
		    ...
		    classdb.actualiza('peticiones_automaticas', criterio, solicitud, 'accounts/getVariousContents/revisaLast', function(actualizado) {
			if (actualizado === 'error') {
			    console.log('accounts/getVariousContents/revisaLast - error al actualizar peticion automática');
...
function procesaVariousContents() {
    var solicitudes_posibles = ['/getom', '/getdm', '/getpb', '/gettt', '/getol', '/gettl', '/getof', '/getoc', '/getos'];
