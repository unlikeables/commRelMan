...
function obtieneTerminosDelTwit(daterms, datracks, datwit, daquote, daretweet, index, matching_arr, callback) {
    var more = index+1;
	...
	setImmediate(function(){
		...
		var termdeturn = datracks[index];
		if (daretweet) {
		    if (daretweet.indexOf(terminodeturno) >= 0) {
			matching_arr.push('retweeted_status:'+termdeturn);
			return obtieneTerminosDelTwit(daterms, datracks, datwit, daquote, daretweet, more, matching_arr, callback);
		    }
		    else {
			if (daquote) {
			    if (daquote.indexOf(terminodeturno) >= 0) {
				matching_arr.push('quoted_status:'+termdeturn);
				return obtieneTerminosDelTwit(daterms, datracks, datwit, daquote, daretweet, more, matching_arr, callback);
			    }
			    else {
				return obtieneTerminosDelTwit(daterms, datracks, datwit, daquote, daretweet, more, matching_arr, callback);
...
function clasifica(untweet, losdatos, callback){
    ...
    obtieneTerminosDelTwit(tracklc, track, eltwitlc, elquoted, elretweeted, 0, [], function(terms_del_twit){
...
getAccountsConTrackers(function(ctascontrackers){
	...
	getFirstArrays(ctascontrackers, 0, [], {}, [], [], dataectas, function(losdatos) {
		...
		stream.on('tweet', function(tweet) {
			...
			if (tuit === 'error') {
			    console.log('el tweet no corresponde a ningun termino, tal vez sea la descripcción del twitero');
			}
			else {
