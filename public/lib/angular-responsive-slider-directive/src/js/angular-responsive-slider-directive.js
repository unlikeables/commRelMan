...
app.directive("ovtsSlider", function($document, $timeout, $http, Authentication) {
  return {
    restrict: "A",
    scope: {
      model: "=",
    ...
    replace: true,
    template: "<div class='slider'><p class='slider-label'>{{label}}</p><p class='slider-value'>{{valuePrefix | formatNumber}}{{displayValue | formatNumber}}{{valueSuffix | formatNumber}}</p><div class='slider-container'><div class='slider-bar'><div class='slider-bar-highlight' ng-style='highlightStyle'></div></div><div class='slider-bar-stub'></div><div class='slider-handle' ng-mousedown='engageHandle($event)' ng-style='handleStyle' title='{{displayValue | formatNumber}}'></div></div></div>",
    link: function(scope, element, attrs) {
      ...
      scope.min = options.min || 10000;
      scope.max = options.max || 1000000;
      ...
      scope.engageHandle = function(event) {
        ...
        mouseMove = function(event) {
          ...
          if(scope.displayValue <10000){
            scope.displayValue = 10000;
          }
          return moveHandle();
        };
        ...
        mouseUp = function() {
          ...
          $http.post('/actualizaRangoInfluencers', {'nombreSistema' : Authentication.user.cuenta.marca, 'total':scope.displayValue}).success(function(respuesta){});
