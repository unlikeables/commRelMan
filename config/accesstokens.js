...
/**
 * Función para obtener un accessToken de entre todos nuestros usuarios 
 */

exports.obtieneUsrsATAmongAllUsrs = function(prefix, cwlbwck) {
  function requestGraph2 (path, callback) {
    if (typeof path === 'undefined' || path === null || path === '') {
      console.log('accesstokens/obtieneUsrsATAmongAllUsrs/requestGraph2 - no hubo path, así que no se puede pedir nada al graph');
      return callback('error');
    }
    else {
      ...
      var solicitud = https.request(globales.options_graph, function(resp) {
			    ...
			    console.log('accesstokens/obtieneUsrsATAmongAllUsrs/requestGraph2.error: '+path);
			    console.log(JSON.stringify(contenido.error));
			    if (typeof contenido.error.code !== 'undefined' && contenido.error.code === 190) {
	...
	console.log('accesstokens/obtieneUsrsATAmongAllUsrs/requestGraph2 - hubo un error en la conexión al graph: '+e);
	return callback('error');
      });
    }
  }
  ...
  function obtieneAccessTokensValidosDeTodos (callback) {
    ...
    classdb.buscarToArray('verifica_auto_uat', critere, lesort, 'rtus/index/obtieneAccessTokensValidosDeTodos', function(items){
      if (items === 'error') {
	...
	if (items.length < 1) {
	  ...
	  obtieneUsuariosConAccessToken(function(usuarios){
		...
		if (validtokens.length < 1) {
		  // no hay tokens válidos para esta cuenta
		  return callback(lostokensvalidos);
		}
		else {
		  var objeto = {
	  ...
	  if (tsm1 < ahora) {
		...
		tokensDeUsuarios(usuarios, 0, [], function(validtokens){
		  if (validtokens.length < 1) {
		    ...
		    lostokensvalidos = validtokens;
		    classdb.actualiza('verifica_auto_uat', critere, objeto, 'solicitudes/getautocp/obtieneAccessTokensValidosDeTodos', function(insertao){
		      return callback(lostokensvalidos);
		    });
		  }
		});
  ...
  obtieneAccessTokensValidosDeTodos(function(losats){
    if (losats.length < 1) {
      return cwlbwck('error');
    }
    else {
      var at_elegido = losats[Math.floor(Math.random()*losats.length)];
      if (prefix === 'no') {
        return cwlbwck(at_elegido);
      }
      else {
        return cwlbwck('access_token='+at_elegido);
      }
    }
  });
};
 ...
 **/
exports.obtieneUsrsAT = function(account, prefix, cewlbewck) {
 ...
 */
exports.obtienePagATByUsrAT = function(cuenta, page_id, user_aid, fb_usid, us_token, prefix, cuwlbuwck) {
  /**
  ...
  getUserPageAT(cuenta, page_id, id_de_usuario, fb_usid, us_token, function(pagtoken){
    if (pagtoken === 'error') {
      ...
      if (prefix === 'no') {
        ...
        return cuwlbuwck('access_token='+pagtoken)
      }
    }
  });
